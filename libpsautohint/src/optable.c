...
init_ops()
{
    ...
    op_table = (struct op_element*)AllocateMem(
      KNOWNOPSIZE, sizeof(struct op_element), "operator table");
    ...
    if (ix > KNOWNOPSIZE) {
        LogMsg(LOGERROR, NONFATALERROR, "Initialization of op table failed.\n");
    }
}
