...
void
get_filename(char* name, char* str, const char* basename)
{
...
bool
FileExists(const char* filename, int16_t errormsg)
{
    struct stat stbuff;
    int filedesc;
...
bool
CFileExists(const char* filename, int16_t errormsg)
{
    return FileExists(filename, errormsg);
}

bool
DirExists(char* dirname, bool absolute, bool create, bool errormsg)
{
#ifndef _WIN32
...
uint32_t ACGetFileSize(filename) char* filename;
{
...
void MoveDerivedFile(filename, basedir, fromDir, toDir) char *filename,
  *basedir, *fromDir, *toDir;
{
...
char* CheckBFPath(baseFontPath) char* baseFontPath;
{
  ...
  a relative path name.\n",
    ...
    for (oldsp = startpath; startpath < oldsp + length; startpath++)
        if (*startpath == *Delimiter)
            *startpath = *Delimiter;
    startpath = AllocateMem(((unsigned int)strlen(newpath)) + length + 1,
                            sizeof(char), "return string for CheckBFPath");
...
void
GetFullPathname(char* dirname, int16_t vRefNum, int32_t dirID)
{
#ifndef _WIN32
...
bool createlockfile(fileToOpen, baseFontPath) char* fileToOpen;
char* baseFontPath; /* for error msg only; cd has been done */
{
    ...
    if (bf == NULL) {
        sprintf(globmsg, "Cannot open the %s%s file.\n", baseFontPath,
                fileToOpen);
        LogMsg(globmsg, LOGERROR, OK, true);
        return false;
    }
...
char *ibmfontname;
{
  ...
  else
  {
    ...
    {
      sprintf(globmsg, "PCFileNamePrefix in the %s file must be exactly 5
characters.\n", FIFILENAME);
      LogMsg(globmsg, LOGERROR, NONFATALERROR, true);
    }
...
int
bf_alphasort(const struct direct** f1, const struct direct** f2)
{
...
int
BFscandir(char* dirName, struct direct*** nameList, includeFile IncludeFile,
          sortFn Sort)
{
    int fail = 0;
    int count = 0;
    ...
    searchstring = (char*)AllocateMem(6 + ((unsigned int)strlen(dirName)),
                                      sizeof(char), "scandir dirname");
    strcpy(searchstring, ".\\");
    ...
    if (count > 0) {
        int i;
        *nameList = (struct direct**)AllocateMem(count, sizeof(struct direct*),
                                                 "scandir namelist");
        for (i = 0; i < count; i++) {
            (*nameList)[i] = (struct direct*)AllocateMem(
              1, sizeof(struct direct), "scandir direct");
        }
    }
    count = 0;
...
#else
int
BFscandir(const char* dirName, struct direct*** nameList,
